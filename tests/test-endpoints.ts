...
async function testCors(name: string, path: string, origin: string) {
  try {
    const res = await fetch(`${BASE_URL}${path}`, {
      method: 'OPTIONS',
      headers: {
...
async function runTests() {
  ...
  await testEndpoint(
    'GET /api/post/:slug (valid format, may 404)',
    '/api/post/test-post',
    404,
    isErrorResponse,
  )
  ...
  if (passed < total) {
    ...
    const nonCorsFailures = results.filter(
      r => !r.passed && !r.name.startsWith('CORS'),
    )
